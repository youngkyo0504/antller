...
function WorkItem({ isHome, id, title, subCategory, index }: WorkItemProps) {
  return (
    <Link passHref href={`/work/${id}`} scroll={false}>
      <a href="">
        <Container css={[isHome ? tw`lg:h-full` : ""]} className="group">
          {/* Image Container */}
          <div
            className="group"
            tw="w-full relative block lg:h-full   after:(transition-opacity top-0 left-0 content w-full h-full absolute bg-black opacity-0 z-20 group-hover:opacity-10 block  duration-[400ms] transition-timing-function[cubic-bezier(0.4, 0, 0.25, 1)])"
          >
            <ImageContainer>
              <Image
